...
export class BoardComponent {
  ...
  constructor(private boardLiveService: BoardLiveService, private boardServices: BoardService, private dialog: MatDialog) { }



  taskDrop(event: CdkDragDrop<any[]>): void {
    // moveItemInArray(tasks, event.previousIndex, event.currentIndex);

    // // Update tasks in the backend after reordering
    // this.boardLiveService.updateTasks(this.board.id, tasks).subscribe({
  ...
  handleDelete(): void {
    ...
    //   if (confirmDelete) {
    //     this.boardLiveService.deleteBoard(this.board.id).subscribe({
